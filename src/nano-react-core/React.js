...
export let errorBoundary = null; // 全局的错误捕获器
export let rejectHandler = null; // 修改全局的promise状态的
export let globalPromise = null; // 全局的promise ，用于捕获错误
 ...
 */
/**
 * 调和子节点数组，初始化时为对每个Fiber节点进行构造，更新时是对每个Fiber节点添加一些新属性 diff结束是beginWork阶段 后面是根据标签判断是不是要复用
...
function reconcileChildren(fiberNode, children) {
  ...
  children.forEach((child, index) => {
    let curFiberNode;
    //判断是否需要更新 初始的创建阶段不会走这个分支 因为不可能有oldFiberChild
    if (isNeedUpdate(oldFiberChild, child)) {
      ...
      //       props: oldFiberChild.props,
      //       child: null,
      //       parent: parentFiberNode, //父节点
      //       sibling: null, // 只是复用节点的值这个不能复用
      //       dom: oldFiberChild.dom,
      //       alternate: oldFiberChild,
      //       effectTag: "update"
      //     }
      ...
      //是否需要删除 存在就删(这里应该是diff) -- 这里收集其实有些草率 -- 只要存在老的节点就删
      if (oldFiberChild) {
...
//更新和初始化函数组件，函数式组件没有自己的dom，所以不需要像updateHostComponent那样考虑真实dom，只跟根据f()=>传入props得到孩子节点
...
//更新和初始化基本元素(一般是div span标签才会走到这一步，为了将情况简化，就只设置了函数式组件和简单组件)
function updateHostComponent(fiberNode) {
  ...
  if (fiberNode.dom === null) {
    ...
    //挂载 这里不再每次处理一个Fiber就挂载一次，当Fiber生成完成后 一次性完成挂载  commitRoot
    // 源码时将子树所有节点加入到instanceDom中，然后将所有子树一起更新props（源码中使用得太复杂，针对不同标签做不同的事件处理，这里我们使用workinProgress的机制）
    // 反正workinProgress也要指向每一个构建的节点，再构建节点的时候更新（创建）它的props
    // map的时候有undifine 因为fiberNode的类型式null
    if(dom!==undefined && fiberNode.props !== null && fiberNode.props){
...
export function updateProps(dom, nextProps, prevProps) {
  ...
  // 情况分类：  
  // 1. next 没有，pre有——>删 删了dom自然不需要管绑定的事件
    ...
    if (key !== 'children') {
      if (nextProps[key] !== prevProps[key]) {
        if (key.startsWith("on", 0)) { //通过是否有on开头的props判断是否需要绑定事件
          // 这种方式实际上和React中的合成事件不同, 后续深入了解尝试替换，本质上针对不通的标签，触发了不通的绑定机制，但是实在是太多了，用原生的替换
...
export function performWorkOfUnit(fiberNode) {
  ...
  if (isFunctionComponent) {
     // 因为fiber是链式结构 什么时候workinProgess构造完了一个组件呢，当这个组件走完了那他一定会走到这个组件的兄弟节点，如果下一个要渲染的节点的类型和workinProgress相同，就将workInProgress =null
...
export function useState(initialValue) {
  ...
  //调用Hook.queue 中的 action 保证函数传参状态的正确性 因为这里我更新了状态值，只是没有提交给workLoop，因此状态在这不会改变
  ...
  function setState(action) {
    ...
    // 交换 innerFuncFiber 和 nextRootFiber  alternate
...
export function useEffect(callback, deps) {
  //初始化effectHook对象，保存回调以及依赖项 useEffect是异步的，将其加入到effectHook对象中
  const effectHook = {
    callback,
    deps,
    cleanup: undefined
...
/**
 * 希望维护一个全局的状态，根据处理函数修改全局的状态
 * 最开始的想法是根据useState 注入到函数组件当中去，来维护，但是当渲染阶段的时候发生了错误之后，自己也拿不到状态信息，因为他不会触发更新
 * 如果维护一个全局的promise，无论在那个阶段被捕获，修改promise的状态，如果promise的状态并为改变的话，说明没有发生错误，否则将错误暴露出来
 * 具体来说，我希望在函数式组件进入的时候给其挂载事件监听器，但是函数式的fiber节点没有dom，所以考虑挂载在全局上--no 还不如直接回调好一点
 * @returns 
 */
export function useError(callback){
  // let innerFuncFiber = functionComponentRoot
  // let eventType = Symbol()
  // document.addEventListener(eventType)
  // innerFuncFiber.dom.addEventListener()
  // 初始化全局的错误promise 以及 全局的错误处理器 状态是 pending
  globalPromise = new Promise((resolve, reject)=>rejectHandler=reject)
  function handleError(stage ,errorInfomation){
    // 在某个阶段的错误捕获  一般是render 和 commit阶段 -- 事件回调是捕获不到的
    rejectHandler({stage, errorInfomation})
    globalPromise.then(()=>{}, callback)
  }
...
export function useRef(initialValue){
  let innerFuncFiber = functionComponentRoot;
  const oldRef = innerFuncFiber.alternate?.refHooks[refIndex]
  // 如果oldRef有值 要忽略它的初始化
  if(oldRef) {
    refIndex++
    return oldRef
  ...
  refIndex++
  refHooks.push(RefObj)
  innerFuncFiber.refHooks = refHooks
  return RefObj
}
